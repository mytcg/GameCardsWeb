function WORK_Shop(){
    ...
    WORK_Shop.prototype.init=function(sXML){
    	...
    	var iWidthOfList = ZS.pageCount*834;
      ...
      $(ZS.divScroll).css({ bottom:3,left:0,width:"100%",height:20 });
    ...
    WORK_Shop.prototype.buildList=function(){
      var divBody=document.getElementsByTagName("body")[0];
      var sXML = ZS.sXML;
      var blockInterval = 198;
      var perRow = ZS.perPage;
      var itemCount = 0;
      for(a = 0;a < ZS.pageCount; a++){
        var pageBlock = ZA.createDiv(ZS.divList,"","","div");
        $(pageBlock).css({ width:834,height:160,position:"relative",cssFloat:"left",styleFloat:"left"});
        ...
        for(b = 0;b < ZS.perPage; b++){
          if (ZA.getXML(ZS.sXML,"pack_"+itemCount+"/id")){
            ...
            if (b < perRow)
              $(itemBlock).css({ top:6,left:(b*blockInterval)+25 });
            else
              $(itemBlock).css({ top:105,left:((b-perRow)*blockInterval)+25 });
    ...
    WORK_Shop.prototype.buyResponse=function(xml){
      ...
      switch(parseInt(ZA.getXML(xml,"value")))
      {
        ...
        case 1:
          ...
          $(divPreview).css({ top:34,left:400,width:250,height:350,background:"url(_site/blankcard.jpg) no-repeat" });
    ...
    WORK_Shop.prototype.buildScroller=function(){
      ...
      var divArrowLeft = ZA.createDiv(ZS.divScroll,"controlspageleft","","div");
      ...
      var divPageCountList = ZA.createDiv(ZS.divScroll,"scroller","","div");
      var offsetCount = (834-(ZS.pageCount*14))/2;
      var iLeft = offsetCount-14;
      $(divPageCountList).css({ top:0,left:20,width:"95%",height:20});
      ...
      ZS.gotoPage(0);
      var divArrowRight = ZA.createDiv(ZS.divScroll,"controlspageright","","div");
    ...
    WORK_Shop.prototype.gotoPage=function(page){
      for (i=0;i<ZS.pageCount;i++){
        $(ZS.divScroll.childNodes[1].childNodes[i]).css({ backgroundPosition:"-441px -60px" }); 
      }
      $(ZS.divScroll.childNodes[1].childNodes[page]).css({ backgroundPosition:"-421px -60px" });
      var newPos = page*-834;
