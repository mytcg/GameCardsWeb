...
function objPlayerBar(){
  ...
  objPlayerBar.prototype.create=function(credits,xp){
    ...
    objBar.iXP = (xp >= 0) ? xp : 0 ;
    objBar.iCredits = (credits >= 0) ? credits : 0 ;
    objBar.iLevel = Math.floor(Math.sqrt(objBar.iXP/50 + 0.25) + 0.5);
    var needed = objBar.iLevel*(objBar.iLevel+1)*50;
    var lastLevel = objBar.iLevel - 1;
    if (lastLevel == 0)
      var diff = objBar.iXP / needed;
    else{
      var LastNeeded = lastLevel*(lastLevel+1)*50;
      var diff = (objBar.iXP - LastNeeded) / (needed-LastNeeded);
    }
    ...
    $(objBar.divBar).css({zIndex:2,top:4,left:4,width:169,height:51,position:"absolute"});
    $(objBar.divBar).draggable();
    var bgImage = ZA.createDiv(objBar.divBar,"","","div");
    $(bgImage).css({top:0,left:0,zIndex:-1,width:169,height:51,position:"absolute",backgroundImage:"url(_site/all.png)",backgroundPosition:"-607px 0px"});
    
    //Create Player level text
    objBar.divLevel = ZA.createDiv(objBar.divBar,"","","div");
    $(objBar.divLevel).css({textAlign:"left",fontWeight:"bold",top:4,left:8,width:59,height:13,position:"absolute"});
    $(objBar.divLevel).html("Level <span id='spanLevelID'>"+objBar.iLevel+"</span>");
    objBar.spanLevelNr = document.getElementById("spanLevelID");
    //Create XP numbers
    objBar.divXP = ZA.createDiv(objBar.divBar,"","","div");
    $(objBar.divXP).css({textAlign:"right",color:"#FFF",fontWeight:"bold",textAlign:"right",top:2,left:70,width:95,height:13,position:"absolute"});
    $(objBar.divXP).html(objBar.iXP+" / "+needed);
    //Create XP bar and percentage
    objBar.divXPbar = ZA.createDiv(objBar.divBar,"","","div");
    $(objBar.divXPbar).css({zIndex:-2,backgroundColor:"#97CC29",top:15,left:7,width:pxIncrease,height:13,position:"absolute"});
    objBar.divXPPercent = ZA.createDiv(objBar.divBar,"","","div");
    $(objBar.divXPPercent).css({fontWeight:"bold",top:31,left:140,width:20,height:13,position:"absolute"});
    $(objBar.divXPPercent).html(diff+"%");
    //Create Credits Bar
    objBar.divCredits = ZA.createDiv(objBar.divBar,"","","div");
    $(objBar.divCredits).css({color:"#FFF",fontWeight:"bold",top:33,left:10,width:120,height:13,position:"absolute"});
    $(objBar.divCredits).html("<span id='spanCreditsID'>Your credits: "+objBar.iCredits+"</span> <span class='txtBlue'>TCG</span>");
    objBar.spanCredits = document.getElementById("spanCreditsID");

$("#spanCreditsID").click(function(){
	ZA.callAjax("_app/?session=1",function(xml){ 
		alert(xml);
	});
});
  ...
  objPlayerBar.prototype.update=function(objProps){
    if (objProps.credits){
      ...
      $(ZA.oPlayerBar.spanCredits).html("Your credits: "+objProps.credits);
      $(ZA.oPlayerBar.spanCredits).css({color:hexColor});
      $(ZA.oPlayerBar.spanCredits).delay(600).animate({color:"#FFF"},1500);
    }
    if (objProps.xp){
...
WORK_App.prototype.createPage=function(){
	...
	divContainer.style.width=ZA.iWidthContainer+"px";
	...
	var divText2 = ZA.createDiv(divCompletion,"divtext2","divtext2");
	...
	ZA.aComponents[5].create(5,"330px");
	...
	ZA.aComponents[8].create(8,"850px,420px,140px,100px");
   ...
   ZA.aComponents[10].create(10,"515px,-50px,75px,55px");
	ZA.drawComponents();
	ZA.createWindow(2,"Shop");
	if (!ZA.sUsername) {
		//not logged in
		ZA.createWindow(3,"Register");
		ZA.startJS("register");
	} else {
		//logged in
		ZA.createWindow(3,"My Deck");
		ZA.startJS("deck");
	}
	ZA.createWindow(5,"Auction");
...
WORK_App.prototype.createWindow=function(iComponentNo,sName,sPos){
	...
	$(divWin).css({left:aPos[0],top:aPos[1],width:aPos[2]
		,height:aPos[3],opacity:0});
	...
	$(divTop).css({height:ZA.iHeightWindowTitle+"px"});
	var divCenter=ZA.createDiv
		(divLeft,"windowleftcenter","windowleft_"+iComponentNo);
	$(divCenter).css({top:ZA.iHeightWindowTitle+"px"
		,height:(iMaxHeight-ZA.iHeightWindowTitle-ZA.iSizeWindowDecor)+"px"});
	...
	if (sName[0]==" ") {
		...
		//fancy title
		var sClassName=("windowtitledesc"+sName.toLowerCase()).replace(/ /g,"");
		var divDesc=ZA.createDiv(divTitle,sClassName,"windowtitledesc_"+iComponentNo);		
	}
	if (iControls) {
		if(!ZA.sUsername){
			if(iComponentNo!=3){
			var divMaximize=ZA.createDiv(divTitle,"windowmaximize1");
			divMaximize.onclick=ZA.maximizeWindow(iComponentNo);
			var divMaximize=ZA.createDiv(divTitle,"windowmaximize2");
			divMaximize.onclick=ZA.maximizeWindow(iComponentNo);
			}
		}else{
			var divMaximize=ZA.createDiv(divTitle,"windowmaximize1");
...
//	divData.style.display="none"
	...
	$(divWin).animate({opacity:ZA.iOpacityWindowInActive},1000);
	$(divData).animate({
		left:aDataPos[0]+"px"
		,top:aDataPos[1]+"px"
		,width:aDataPos[2]+"px"
...
//create popup window
WORK_App.prototype.createWindowPopup=function
	(iComponentNo,sName,iWidth,iHeight,iIsModal,iControls){
	var divBody=document.getElementsByTagName("body")[0];
	if (iIsModal){
      ...
      iBrowserHeight=window.innerHeight;   
		...
		var iCloakHeight = (iBrowserHeight > 715) ? iBrowserHeight : 715;
		$(divCloak).css({display:"block",height:iCloakHeight});
		ZA.setNextZIndex(divCloak);
		$(divCloak).animate({opacity:0.5});
	}
...
WORK_App.prototype.init=function(sXMLInit){
  ...
  /////////////////////////
	...
	ZA.iWidthContainer=parseInt(ZA.getXML(sXMLInit,"containerwidth"));
	ZA.iHeightContainer=parseInt(ZA.getXML(sXMLInit,"containerheight"));
	...
	ZA.iOpacityWindowInActive=ZA.getXML(sXMLInit,"windowopacityinactive");
	...
	$(divHeader).css({height:ZA.iHeightHeader+"px",opacity:ZA.iOpacityHeader});
	var divStats=ZA.createDiv(divHeader,"","stats");
	ZA.createPage();
	var divFooter=ZA.createDiv(divBody,"","bodyfooter");
	$(divFooter).css({color:"#FFF",paddingTop:8,height:ZA.iHeightFooter+"px",opacity:ZA.iOpacityHeader});
	$(divFooter).html("MyTCG Website Version 1.0");
	//ZM.createBottom();
	ZM.createLeft(sXMLInit);
	ZM.createTop();
	ZM.createSocial();
	...
	if(ZA.getXML(sXMLInit,"popup")=="1")
	{
		...
		if(ZA.getXML(sXMLInit,"dailyvisit")=="1"){
			ZA.gotCredits("25 credits for logging in today",ZA.getXML(sXMLInit,"newcredits"));
		}
// 		
		// if(ZA.getXML(sXMLInit,"freebie")=="1"){
// 		  
...
// 		  
      ...
      // $(divHead).css({position:"absolute",top:30,left:130});
      // $(divHead).html("<b>Welcome to MyTCG</b><br>Here is a little something to start you off with.");
// 		  
...
WORK_App.prototype.maximizeWindow=function(iComponentNo){
	return function() {
	  if((iComponentNo!=3)||(ZA.sUsername)){
...
WORK_App.prototype.maximizeWindowA=function(iComponentNo){
	...
	if (ZA.aComponents[iComponentNo].iIsMaximized) {
		...
		$(divComponent).animate({
			left:0+"px"
			,top:ZA.iHeightMargin+"px"
			,width:(ZA.iWidthContainer)+"px"
			,height:(ZA.iHeightContainer-80)+"px"});
		$(divTitle).animate({width:(ZA.iWidthContainer-16)+"px"});
		$(divData).animate({
			width:(ZA.iWidthContainer-16)+"px"
			,height:(ZA.iHeightContainer-125)+"px"});
		$(divWindow).animate({
			width:(ZA.iWidthContainer-0)+"px"
			,height:(ZA.iHeightContainer-80)+"px",opacity:1});
		$(divBottom).animate({width:(ZA.iWidthContainer-16)+"px"});
		$(divTitleBottom).animate({width:(ZA.iWidthContainer-16)+"px"});
		$(divLeft).animate({height:(ZA.iHeightContainer-118)+"px"});
		$(divRight).animate({height:(ZA.iHeightContainer-118)+"px"});
		...
		$(divTitleBottom).animate({
			width:(parseInt(ZA.aComponents[iComponentNo].aPos[2])-16)+"px"});
		$(divLeft).animate({
			height:(parseInt(ZA.aComponents[iComponentNo].aPos[3])-38)+"px"});
		$(divRight).animate({
			height:(parseInt(ZA.aComponents[iComponentNo].aPos[3])-38)+"px"});
...
WORK_App.prototype.showCreditsPopup=function()
{
	//Show credits screen
	ZA.createWindowPopup(9911,"credits",670,440,1,0);
	var divData=document.getElementById("window_9911");
	...
	//title
	var divTitle = ZA.createDiv(divContainer);
	$(divTitle).css({
		width:"100%",
		top:0,
		left:0
	});
	var img = ZA.createDiv(divTitle,"divgotcredits");
	var div = ZA.createDiv(divTitle);
	$(div).css({
		fontSize:16,
		fontWeight:"bold",
		left:120,
    ...
    //current balance
    var info = ZA.createDiv(divContainer);
    $(info).css({
    	width:200,
    	height:86,
    	top:10,
    ...
    $(div).html('Current Balance');
    var div = ZA.createDiv(info,"txtBlue");
    $(div).css({
    	position:"relative",
    	fontSize:16,
    	fontWeight:"bold",
    ...
    $(div).html(ZA.sUserCredits+' TCG');
	
	//logs holder
	var divLogs = ZA.createDiv(divContainer);
	$(divLogs).css({
		top:100,
		width:631,
		margin:10,
	...
	ZA.callAjax("_app/?credits=1",function(xml){
		...
		if(logsCount > 0){
			...
			for(var i=0; i<logsCount; i++){
				...
				signcol = (parseInt(ZA.getXML(xml,"log_"+i+"/amount")) > 0) ? '#6c7927' : '#CC0000';
			...
			var headers = ZA.createDiv(divLogs);
			$(headers).css({
				position:"relative",
				width:"100%",
				color:"#FFF",
			...
			var table = ZA.createDiv(divLogs,"","divCredits");
			$(table).css({
				position:"relative",
				width:"100%",
				height:220,
	...
	$(divButton).click(function(){
		var divBody=document.getElementsByTagName("body")[0];
		var divCloak=document.getElementById("bodycloak_9911");
		var divCredits=document.getElementById("windowcontainer_9911");
		var divData=document.getElementById("window_9911");
		if (divCredits) {
			divBody.removeChild(divCredits);
			divBody.removeChild(divData);
		}
		if(divCloak) {
			divBody.removeChild(divCloak);
...
WORK_Component.prototype.create=function(iComponentNo,sSize){
	...
	switch (iComponentNo) {
		case 1:
			aPos[0]="0px";
			aPos[1]=(ZA.iHeightMargin)+"px";
			aPos[2]=aSize[0];
			aPos[3]=aSize[1];			
		break;
		
		case 2:
			aPos[0]=(parseInt(ZA.aComponents[1].aPos[2]))+ZA.iWidthMargin+"px";
			aPos[1]=(ZA.iHeightMargin)+"px";
			aPos[2]=aSize[0];
			aPos[3]=(parseInt(ZA.aComponents[1].aPos[3]))+"px";
		break;
		
		case 3:
			aPos[0]=(parseInt(ZA.aComponents[1].aPos[2])
				+(parseInt(ZA.aComponents[2].aPos[2])))+(2*ZA.iWidthMargin)+"px";
			aPos[1]=(ZA.iHeightMargin)+"px";
			aPos[2]=ZA.iWidthContainer-parseInt(aPos[0])+"px";
			aPos[3]=(parseInt(ZA.aComponents[1].aPos[3]))+"px";
		break;
		...
		case 5:
			aPos[0]=parseInt(ZA.aComponents[4].aPos[2])+ZA.iWidthMargin+"px";
			aPos[1]=ZA.aComponents[4].aPos[1];
			aPos[2]=aSize[0];
			aPos[3]=ZA.aComponents[4].aPos[3];
		break;
		
		case 6:
			aPos[0]=(parseInt(ZA.aComponents[4].aPos[2])
				+(parseInt(ZA.aComponents[5].aPos[2])))+(2*ZA.iWidthMargin)+"px";
			aPos[1]=ZA.aComponents[4].aPos[1];
			aPos[2]=ZA.iWidthContainer-parseInt(aPos[0])+"px";
			aPos[3]=ZA.aComponents[4].aPos[3];
		break;
...
WORK_Login.prototype.clickForgotPassword=function(){
	return function() {
		...
		$(divForget).css({ width:"350px",height:"220px",backgroundColor:"#DADBDC" });
  ...
  var divClose = ZA.createDiv(divForget,"cmdButton","","div");
    ...
    $(divResponse).css({ top:"80px",left:"8px"});
...
WORK_Menu.prototype.createSocial=function(){
  ...
  hrefHolder.href="https://twitter.com/#!/mytcgtech";
