...
if (intval($_GET['init']==1))
{
	if(strlen($userID) > 0){
		$sql = "SELECT U.username AS 'owner', I.description AS 'imageserver', CA.description AS 'category', C.description, C.image, C.category_id, UC.usercard_id, UC.card_id, M.*,
				...
				JOIN ".$pre."_imageserver I ON C.back_imageserver_id = I.imageserver_id
				JOIN ".$pre."_category CA ON C.category_id = CA.category_id
		...
		//get user credits
		$userQuery = myqu("SELECT credits FROM ".$pre."_user WHERE user_id=".$userID);
...
if(intval($_GET["cat"]) > 0){
  ...
  $xml = "";
  $query = "SELECT U.username AS 'owner', ISC.description AS 'thumbnail_imageserver', ISB.description AS 'front_imageserver', ISA.description AS 'back_imageserver', CA.description AS 'category', C.description, C.image, C.category_id, UC.usercard_id, UC.card_id, M.*,
  			(SELECT COUNT(usercard_id) FROM mytcg_usercard WHERE user_id=".$userID." AND card_id=UC.card_id AND usercardstatus_id=1) AS 'owned' 
      FROM ".$pre."_market M
      JOIN ".$pre."_usercard UC USING (usercard_id)
      JOIN ".$pre."_card C USING (card_id)
	  JOIN ".$pre."_imageserver ISA ON C.back_imageserver_id = ISA.imageserver_id
	  JOIN ".$pre."_imageserver ISB ON C.front_imageserver_id = ISB.imageserver_id
	  JOIN ".$pre."_imageserver ISC ON C.thumbnail_imageserver_id = ISC.imageserver_id
      JOIN ".$pre."_category CA ON C.category_id = CA.category_id
      JOIN ".$pre."_user U ON M.user_id = U.user_id
  ...
  $sql = "SELECT credits FROM ".$pre."_user WHERE user_id=".$userID.";";
  $usr = myqu($sql);
  $usr = $usr[0];
  ...
  $carName = $carName[0]['description'];
	...
	if(sizeof($lastBidderQuery) > 0){
		foreach($lastBidderQuery as $lastBidder){
			myqu("UPDATE mytcg_user SET credits = credits+".$lastBidder['price']." WHERE user_id = ".$lastBidder['user_id']);
			//add transaction log
	...
	$sql = "UPDATE ".$pre."_user SET credits=(credits-".$price.") WHERE user_id=".$newowner.";";
	$temp[]=$sql;
	myqu($sql);
	//Add transaction log
	myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
			VALUES(".$newowner.", 'Spent ".$price." credits on buyout of ".$carName."', NOW(), -".$price.")");
	
	//Add credits to seller
	$sql = "UPDATE ".$pre."_user SET credits=(credits+".$price.") WHERE user_id=".$oldowner.";";
	$temp[]=$sql;
	myqu($sql);
	//Add transaction log
	myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
	...
	$sql = "SELECT credits FROM ".$pre."_user WHERE user_id=".$userID.";";
	$u = myqu($sql);
	$iCreditsAfterPurchase = $u[0]['credits'];
	$xml = '';
	$xml.= '<response>'.$sCRLF;
...
if (isset($_GET['auction']))
{
	...
	if (isset($_GET['placebid']))
	{
		...
		$query = myqu("SELECT credits FROM ".$pre."_user WHERE user_id = ".$userID);
		$available_credits = $query[0]['credits'];
		...
		if($available_credits < $bid_amount)
		{
			...
			echo $sTab.'<message val="Insuffient credits to place bid. Only '.$available_credits.' TCG credits available." />'.$sCRLF;
			echo '</result>'.$sCRLF;
			exit;
		}
		else
      ...
      $carName = $carName[0]['description'];
			...
			myqu("UPDATE mytcg_user SET credits = credits-".$bid_amount." WHERE user_id = ".$userID);
			//add transaction log
			myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
