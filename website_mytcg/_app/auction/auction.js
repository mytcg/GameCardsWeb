...
WORK_Auction.prototype.loadAuctions=function()
{
	...
	if(ZU.iCellCountAll > 0)
	{
		...
		$(divAuctionsHolder).css({width:"100%"}).show();
		
		while (iCount<(ZU.iCellCountAll))
		{
			var divAuctionBlock = ZA.createDiv(divAuctionsHolder,"auctionBlock",iCount.toString(),"div");
			...
			var divAuctionData = ZA.createDiv(divAuctionBlock);
			$(divAuctionData).css({
				width:"100%",
				height:72,
				bottom:0
			...
			$(divAuctionTitle).html(ZA.getXML(ZU.sXML,"cards/card_"+iCount+"/description"));
			dataTop+=10;
			...
			var divHighestBidder = ZA.createDiv(divAuctionData,"iconHighestBidder",iCount.toString(),"div");
			...
			var divAuctionDetails = ZA.createDiv(divAuctionData,"","","div");
			...
			if(bidCount > 0){
				...
				var bidder = ZA.getXML(ZU.sXML,"cards/card_"+iCount+"/history/bid_0/user");
				if(bidder == 'You'){
			...
			if(mine == '1'){
				...
				owner = 'Seller: '+ZA.getXML(ZU.sXML,"cards/card_"+iCount+"/owner");
			}
			$(divAuctionDetails).html(
				'<span style="color:#999;">'+owner+'</span><br />'+
			...
			var divAuctionLink = ZA.createDiv(divAuctionBlock,"cmdButton viewAuction",iCount.toString(),"div");
			$(divAuctionLink).css({
				left:3,
				bottom:5
			});
			...
			if(buyoutPrice != '0')
			{
				var div = ZA.createDiv(divAuctionBlock,"","","div");
				$(div).css({
					width:20,
					height:20,
					bottom:5,
					right:5,
					background:"url(_site/all.png) -275px -5px no-repeat"
				});
				$(div).attr('title','Buy out available');
				var divBuyout = ZA.createDiv(divAuctionBlock,"","","div");
				$(divBuyout).css({
					bottom:9,
					right:31,
					color:"#cc2222",
				...
				$(divBuyout).html(buyoutPrice+' TCG');
			}
...
WORK_Auction.prototype.loadAuctionsLarge=function()
{
	...
	$(topMenuMain).css({ width:974,height:28,backgroundColor:"transparent","z-index":"9999",position:"relative",padding:5 });
	//Clear button
	...
	if(ZU.iCellCountAll > 0)
	{
		...
		while (iCount<ZU.iCellCountAll)
		{
			...
			var divHighestBidder = ZA.createDiv(divAuctionBlock,"iconHighestBidder",iCount.toString(),"div");
			$(divHighestBidder).attr('title','You are the highest bidder');
			
			var divAuctionDetails = ZA.createDiv(divAuctionBlock,"","","div");
			$(divAuctionDetails).css({
				left:85,
				top:22,
				"line-height":1.5,
			...
			$(divAuctionDetails).html(
				...
				'<span style="color:#999;position:absolute;top:34px;"><nobr>[<span class="bidCount">'+bids+'</span>]</nobr></span>'
			);
			
			var divAuctionLink = ZA.createDiv(divAuctionBlock,"viewAuction",iCount.toString(),"div");
			$(divAuctionLink).css({
				left:85,
				bottom:10
			});
...
WORK_AuctionWindow.prototype.create=function(ID)
{
	...
	ZA.callAjax(ZU.sURL+"?auction=1&market="+auctionID,function(aXML){
		...
		var owner = ZA.getXML(aXML,"details/owner");
		...
		if(bidCount != '0')
		{
			...
			highestBidder = ZA.getXML(aXML,"details/history/bid_0/user");
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					$(div).html(
						'<input type="text" id="bidValue" value="'+yourBid+'" style="-moz-user-select:text;height:25px;text-align:center;border:1px solid #ccc;background:#fff;font-size:16px;font-weight:bold;" size="4" />'+
					...
					{
						//Split line
						var divSplit = ZA.createDiv(divActions,"","","div");
						$(divSplit).css({
							position:"relative",
							height:13
						});
...
WORK_AuctionWindow.prototype.setBiddingHistory=function(aXML){
	...
	if(bidCount > 0)
	{
		...
		{
			if(i > 9){return false;}
			if(i%2==1){rowbg='#FFF';}else{rowbg='#E5EEC3';}
			...
			$(div).html(
				...
				ZA.getXML(aXML,"details/history/bid_"+i+"/user")+'</td><td align="right" width="25">'+
