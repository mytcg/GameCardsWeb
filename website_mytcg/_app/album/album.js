function WORK_Album(){
  ...
  this.pageCount = 0;
  this.perPage = 8;         //Hardcoded value
  this.perRow = 4;        //Hardcoded value
  this.currentPage = 0;
	this.divList = null;
  ...
  this.aWindowAuction = null;
    ...
    WORK_Album.prototype.init=function(sXML){
	  ...
	  else
	  {
	      ...
	      if(albumCount > 0)
	      {
		      ...
		      $(ZL.divMenu).css({ zIndex:5,overflow:"hidden",top:0,right:0,backgroundColor:"#646464" });
				if (ZA.aComponents[ZL.iComponentNo].iIsMaximized) {
					$(ZL.divMenu).css({ height:ZL.iWindowHeight-26,width:170 });
				}
				else{
					$(ZL.divMenu).css({ height:ZL.iWindowHeight-25,width:120 });
				}
    ...
    WORK_Album.prototype.buildMenu=function(){
	  ...
	  //Menu scroller: UP
		var upArrow = ZA.createDiv(divMenu,"scrollArrow","upArrow","div");
		$(upArrow).css({ cursor:"pointer",top:0,left:0,width:"100%",height:12,padding:2,textAlign:"center",color:"#FFF",background:"#646464","z-index":99,"border-bottom":"1px solid black;" });
		$(upArrow).html('&#x25B2;');
	  ...
	  //Menu holder for menu blocks
		var menuHolder = ZA.createDiv(divMenu,"","menuHolder","div");
    ...
    if(newTotal > 0){
      ...
      $(menuBlock).css({ cursor:"pointer",top:0,left:-15,width:"100%",height:10,padding:8,textAlign:"right",color:"#FFF" }); 
    ...
    $(menuBlock).css({ cursor:"pointer",top:(CatCounter*ZL.iMenuBlockHeight),left:-15,width:"100%",height:10,padding:8,textAlign:"right",color:"#FFF" }); 
     ...
     */
     ...
     //Cats
      for (i=0;i<iCount;i++){
        ...
        $(menuBlock).css({ cursor:"pointer",top:iTop,left:-15,width:"100%",height:10,padding:8,textAlign:"right",color:"#FFF" }); 
      ...
      //Menu scroller: DOWN
		var downArrow = ZA.createDiv(divMenu,"scrollArrow","downArrow","div");
		$(downArrow).css({ cursor:"pointer",left:0,top:(parseInt($(divMenu).css('height'),10)-16),width:"100%",height:13,padding:2,textAlign:"center",color:"#FFF",background:"#646464" }); 
		$(downArrow).html('&#x25BC;');
      ...
      $(divLine).css({ top:-2,left:0,width:"100%",height:2,backgroundImage:"url(_site/repeatx.png)",backgroundPosition:"0px -121px" });
    };
    ...
    WORK_Album.prototype.showAlbum=function(albumNr){
      ...
      while(itemCount < ZL.itemCount){
        if (itemRowCount == ZL.perRow) {
          iTop += 115;
          iLeft = (ZL.iWindowWidth-lMod) * iStart + 5;
          itemRowCount = 0;
        }
        ...
        $(shadowBlock).css({ top:0,left:7,width:70,height:95,backgroundImage:ZL.imgAll,backgroundPosition:"-615px -119px" });
	...
	WORK_Album.prototype.clickShowFullImage=function(iAlbumNo, iThumbnailNo){
		...
		*/
		...
		$(divTab).css({
			background:"url(_site/all.png) -735px -60px no-repeat",
			//top:130
			top:70
		});
    ...
    WORK_Album.prototype.buildScroller=function(){
      ...
      ZL.gotoPage(0);
      var divArrowRight = ZA.createDiv(ZL.divScroll,"","albumarrowright","div");
    ...
    WORK_Album.prototype.maximize=function(){
      ...
      $("#downArrow").css('top', parseInt($(ZL.divMenu).css('height')) - 17 );
      if (!ZA.aComponents[ZL.iComponentNo].iIsMaximized) {
		...
		if(confirm(
			...
			function(xml){
				...
				if(result=='success')
				{
		        	...
		        	//Reload auctions
		        	$(ZU.divData).html('<div class="loader"></div>');
		        	ZA.callAjax(ZU.sURL+"?init=1",function(xml){ZU.init(xml);});
