function WORK_Shop(){
    ...
    WORK_Shop.prototype.init=function(sXML){
    	...
    	$(ZS.divList).css({ top:0,left:0,width:iWidthOfList,height:204,backgroundColor:"#666" });
    	
    	ZS.divScroll = ZA.createDiv(ZS.divData,"","divScroll","div");
      $(ZS.divScroll).css({ top:209,left:0,width:484,height:20 });
    ...
    WORK_Shop.prototype.buildListLarge=function(){
      ...
      $(ZS.divListLarge).css({ display:"none",opacity:0,position:"relative",width:984,height:535 });
      ...
      while(itemCount < iProducts){
        ...
        $(itemBlock).css({ backgroundColor:"#EFEFEF","position":"relative","float":"left",width:230,height:135,"margin-left":8,"margin-top":8,"border":"1px solid #ccc"});
        ...
        divLeft.src = ZA.getXML(ZS.sXML,"pack_"+itemCount+"/fullimageserver")+"products/"+ZA.getXML(ZS.sXML,"pack_"+itemCount+"/img")+".png";
		...
		var credits = ZA.getXML(ZS.sXML,"pack_"+itemCount+"/price");
		var premium = ZA.getXML(ZS.sXML,"pack_"+itemCount+"/premiumprice");
		var description = ZA.getXML(ZS.sXML,"pack_"+itemCount+"/desc");
		var cards = ZA.getXML(ZS.sXML,"pack_"+itemCount+"/cards");
		
		if(credits > 0){
			var creditType = credits;
			var creditClass = '"txtBlue"';
		}else{
			var creditType = premium;
			var creditClass = '"txtGold"';
		}
        ...
        $(divTop).html(
        	"<b><span class='txtGreen' style='font-size:12px;'>"+description+"</span><br />"+cards+" Cards in pack<br /><span class="+creditClass+" style=\"font-size:16px;\">"+creditType+" TCG</span></b>"
    ...
    WORK_Shop.prototype.viewPackCards=function(xml)
    {
        ...
        $(image).html('<img src="'+ZA.getXML(xml,"path")+'products/'+ZA.getXML(xml,"image")+'.png" />');
    ...
    WORK_Shop.prototype.buildList=function(){
      ...
      var perRow = ZS.perPage/2;
      var itemCount = 0;
      for(a = 0;a < ZS.pageCount; a++){
        var pageBlock = ZA.createDiv(ZS.divList,"","","div");
        $(pageBlock).css({ width:484,height:204,position:"relative",cssFloat:"left",styleFloat:"left" });
        ...
        for(b = 0;b < ZS.perPage; b++){
          if (ZA.getXML(ZS.sXML,"pack_"+itemCount+"/id")){
            ...
            $(itemBlock).css({ backgroundImage:"url("+sImgPath+")",backgroundPosition:ZA.getXML(ZS.sXML,"pack_"+itemCount+"/backgroundposition") }); 
            ...
            */
            var blackBox = ZA.createDiv(itemBlock,"","","div");
            $(blackBox).css({ width:152,height:42,top:50,opacity:0.8,backgroundColor:"#000" });
            var textBox = ZA.createDiv(itemBlock,"","","div");
            $(textBox).css({ paddingTop:2,paddingLeft:6,width:92,height:38,top:50,color:"#FFF",textAlign:"left" });          
            ...
            $(textBox).html("<span class='txtGreen'>"+ZA.getXML(ZS.sXML,"pack_"+itemCount+"/desc")+"</span><br />"+ZA.getXML(ZS.sXML,"pack_"+itemCount+"/cards")+" cards - TCG <span class="+creditClass+">"+creditType+"</span>");
            
            var buyButton = ZA.createDiv(itemBlock,"","","div");
            $(buyButton).css({ zIndex:4,top:61,left:107 });
    ...
    WORK_Shop.prototype.buyResponse=function(xml){
      ...
      {
        case -2:
          response = "Sorry but but they are not for sale yet.";
        break;
        case -1:
        ...
        case 1:
              ...
              $(imgBlock).click({p:imgPath,i:imgID,b:cardBlock},function(event){
                 ...
                 var i = event.data.i;
             	 $("#imgFront").attr("src","").hide('fade',150,function(){
                 	...
                 	$("#imgFront").attr("src",p+"cards/"+i+"_front.png").show();
             	 });
                 $("#imgBack").hide('fade',150,function(){
                 	$(this).hide();
                 	$("#imgBack").attr("src",p+"cards/"+i+"_back.png");
                 });
              });
          ...
          $(divPreview).css({ top:34,left:400,width:250,height:350,background:"url(img/cards/gc.png) no-repeat" });
    ...
    WORK_Shop.prototype.gotoPage=function(page){
      for (i=0;i<ZS.pageCount;i++){
        $(ZS.divScroll.childNodes[1].childNodes[i]).css({ backgroundPosition:"-281px -41px" }); 
      }
      $(ZS.divScroll.childNodes[1].childNodes[page]).css({ backgroundPosition:"-291px -41px" });
      var newPos = page*-484;
