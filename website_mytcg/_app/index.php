...
if (intval($_GET["login"])==1){
	...
	$sql =  "SELECT user_id, username, password, date_last_visit,mobile_date_last_visit, credits, xp, freebie, completion_process_stage, category_id "
			."FROM mytcg_user "
    ...
    $ip = getip();
    $sUA=myqu("UPDATE mytcg_user SET last_useragent='".$sUA."',ip = '".$ip."' WHERE user_id='".$iValidUserID."'");
	...
	$_SESSION['user']['premuim']=$aValidUser[0]['premuim'];
    ...
    if($_GET['r']=="true"){
      ...
      setcookie("rLogin", $sDetails, $expire,"/");
		...
		echo $sTab.'<credits val="'.$aValidUser[0]['credits'].'" />'.$sCRLF;
  ...
  echo $sTab.'<menuleft val="'
    ...
    if(($cps=='6')&&($sMobileLastDate!="")){
      ...
      $cps = "7";
    	...
    	echo $sTab.'<select_category val="'.$aUser['category_id'].'" />'.$sCRLF;
		...
		if((substr($sLastDate,0,10) != $today)&&(substr($sMobileLastDate,0,10) != $today))
		{
			...
			myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
					VALUES(".$userID.", 'Received 25 credits for logging in today', NOW(), 50)");
		...
		if(($aUser['freebie']=='0') || (is_null($aUser['freebie']))){
			...
			$starterPacks = myqu("SELECT P.product_id, P.description, CONCAT(IMG.description,'products/',P.image,'.png') AS 'image'
