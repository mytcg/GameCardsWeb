...
WORK_Toptrumps.prototype.showGameMenu=function()
{
	...
	var div = ZA.createDiv(divHolder);
	$(div).css({
		background:"#fff",
		height:350,
		width:640,
	...
	$(divScreen).show();
		...
		var div = ZA.createDiv(divScreen);
		$(div).css({
			position:"relative",
			width:224,
			height:190,
			...
			marginLeft:"auto",
			marginRight:"auto",
			background:"transparent url(_site/gameboard.png) 0px -688px no-repeat"
		});
		var divMenuItems = ZA.createDiv(divScreen);
		$(divMenuItems).css({
			width:248,
			left:200,
			top:155,
	...
	var divScreen = ZA.createDiv(divHolder,"gameScreen","settings");
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -639px -780px no-repeat",
			width:109
		});
	...
	var divScreen = ZA.createDiv(divHolder,"gameScreen","category");
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -239px -700px no-repeat"
		});
	...
	var divScreen = ZA.createDiv(divHolder,"gameScreen","opponent");
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -559px -700px no-repeat"
		});
		
			//Opponent options
			var divOpts = ZA.createDiv(divScreen);
			$(divOpts).css({
				position:"relative",
				height:80,
				width:540,
			...
			//Computer
			var divBlock = ZA.createDiv(divOpts,"versusOptionBlockHolder");
	...
	var divScreen = ZA.createDiv(divHolder,"gameScreen","difficulty");
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -239px -780px no-repeat",
			width:194
		});
			...
			//info text
			var div = ZA.createDiv(divScreen,"txtBlue","difficultyInfo");
			$(div).css({
				paddingTop:30,
				width:"100%",
				fontWeight:"bold"
...
WORK_Toptrumps.prototype.buildDeckScreen=function(gameScreen)
{
	...
	var divScreen = ZA.createDiv(ZT.divGameScreens,"gameScreen","deck");
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -399px -700px no-repeat",
			marginBottom:10
		});
			...
			$(div).html(
				'<table style="width:500px" align="center">'+
					'<tr>'+
						'<td style="width:250px" align="right">CATEGORY:</td>'+
						'<td style="width:250px" align="left"><strong>'+$(".categoryOptions").find(".categoryOptionSelected").attr('title')+'</strong></td>'+
					'</tr>'+
				'</table>'
			);
			
			// Get and display user's decks
			var divDecksHolder = ZA.createDiv(divScreen,"userDecksHolder");
			var div = ZA.createDiv(divDecksHolder);
			$(div).css({
				background:"#fff",
				width:"100%",
				height:"100%",
			...
			for(i=0; i<parseInt(ZA.getXML(ZD.sXML,"deckcount")); i++){
				if(ZA.getXML(ZD.sXML,"decks/deck_"+i+"/categoryid") == ZT.playerCategoryID){
					...
					if(cardcount == 10){
						...
						$(divDeck).css({
							background:"url("+ZA.getXML(ZD.sXML,"decks/deck_"+i+"/image")+") center center no-repeat"
						});
						var div = ZA.createDiv(divDeck);
						$(div).css({
							background:"#000",
							color:"#fff",
							bottom:0,
						...
						$(div).html(ZA.getXML(ZD.sXML,"decks/deck_"+i+"/description"));
						var div = ZA.createDiv(divDeck);
						$(div).css({
							color:"#fff",
							top:4,
			...
			var nodecks = false;
			if(deckCount == 0){
				$(divDecks).html('<div style="margin-top:40px;width:100%;">No full decks found. Your 10 highest ranked cards will play.</div>');
				ZT.deckPages = 1;
				ZT.playerDeckID = -1;
				nodecks = true;
			}
			...
			//Scroller
			var divScroll = ZA.createDiv(divDecksHolder,"divscroll","","div");
			$(divScroll).css({
				width:466,
				height:20,
				left:0,
			...
			$(divArrowLeft).css({
				background:"url(_site/all.png) -221px -41px no-repeat",
				width:20,
				height:20,
				left:10,
				bottom:0,
			...
			$(divArrowRight).css({
				background:"url(_site/all.png) -251px -41px no-repeat",
				width:20,
				height:20,
				right:10,
				bottom:0,
...
WORK_Toptrumps.prototype.buildLoadScreen=function()
{
	...
	ZA.callAjax(ZT.sURL+'?savedgames=1',function(xml)
	{
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -479px -780px no-repeat",
			width:96
		});
		
		var divGames = ZA.createDiv(divScreen,"savedGames");
		$(divGames).css({
			position:"relative",
			height:120,
			width:340,
...
WORK_Toptrumps.prototype.buildPlayerScreen=function()
{
	...
	var divScreen = ZA.createDiv(ZT.divGameScreens,"gameScreen","player");
		...
		var div = ZA.createDiv(divScreen,"gameScreenTitle");
		$(div).css({
			background:"transparent url(_site/gameboard.png) -719px -700px no-repeat",
			width:156,
			marginBottom:10
		});
		...
		$(div).html(
			'<table style="width:500px" align="center">'+
				'<tr>'+
					...
					'<td style="width:250px" align="left"><strong>'+$(".categoryOptions").find(".categoryOptionSelected").attr('title')+'</strong></td>'+
				'</tr><tr>'+
					'<td style="width:250px" align="right">CHOSEN DECK:</td>'+
					'<td style="width:250px" align="left"><strong>'+chosenDeck+'</strong></td>'+
				'</tr>'+
			'</table>'
		);
		
		var divOpp = ZA.createDiv(divScreen,"","gameOpponent");
		$(divOpp).css({
			position:"relative",
			width:"50%",
			height:80,
...
WORK_Toptrumps.prototype.buildFriendScreen=function()
{
	...
	var divScreen = ZA.createDiv(ZT.divGameScreens,"gameScreen","friend");
		...
		var divSearch = ZA.createDiv(divScreen);
		$(divSearch).css({
			position:"relative",
			width:"50%",
			height:90,
	...
	ZA.callAjax(ZT.sURL+'?init=1&game='+ZT.playerGameID+'&opponent='+ZT.playerOpponent+'&category='+ZT.playerCategoryID+'&deck='+deckId+'&difficulty='+ZT.playerDifficulty,function(tXML)
	{
		...
		ZA.callAjax(ZT.sURL+'?cards=1&category='+ZT.playerCategoryID,function(cXML)
		{
			...
			$(divP1).html('<div style="width:100%;height:100%;opacity:0.5;background:black;"></div>');
				...
				//Player 1 avatar
				var divAvatar = ZA.createDiv(divP1,"playerAvatarHolder");
				$(divAvatar).css({
					left:15,
					bottom:15
				});
				...
				$(div).css({
					background:"#ccc url(_site/gameboard.png) no-repeat -216px -38px"
				});
				...
				var divPlayer1Cards = ZA.createDiv(divP1,"playerCards","1");
				$(divPlayer1Cards).css({
					left:15,
					bottom:135
				});
				...
				var divScore = ZA.createDiv(divP1);
				$(divScore).css({
					background:"#000",
				...
				$(divScore).html('<div style="color:#fff;position:relative;font-size:36px;font-weight:bold;padding:18px;" id="score1"></div>');
				...
				for(i=0; i<parseInt(ZT.p1score); i++)
				{
					...
					$(div).css({
						background:"url(img/cards/thumb0.png) no-repeat",
						bottom:(i*12)
					});
				}
			...
			$(divP2).html('<div style="width:100%;height:100%;opacity:0.5;background:black;"></div>');
				...
				//Player 2 avatar
				var divAvatar = ZA.createDiv(divP2,"playerAvatarHolder");
				$(divAvatar).css({
					right:15,
					bottom:15
				});
				var iBgLeft = (ZT.playerOpponent.toString()=='0') ? '-126px' : '-216px';
				var div = ZA.createDiv(divAvatar,"playerAvatar");
				$(div).css({
					right:0,
					background:"#ccc url(_site/gameboard.png) no-repeat "+iBgLeft+" -38px"
				});
				$(div).html('<div id="p2loader"><img src="_site/loading.gif" style="margin:21px;" /></div>');
				...
				var divPlayer2Cards = ZA.createDiv(divP2,"playerCards","2");
				$(divPlayer2Cards).css({
					right:15,
					bottom:135
				});
				...
				var divScore = ZA.createDiv(divP2);
				$(divScore).css({
					background:"#000",
					border:"5px solid #999",
					width:80,
				...
				$(divScore).html('<div style="color:#fff;position:relative;font-size:36px;font-weight:bold;padding:18px;" id="score2"></div>');
			...
			for(var i=0; i<$(gameCardsShuffled).size(); i++){
				var backimage = ZA.getXML(ZT.cardsXML,"card_"+ZT.cardsArray[gameCardsShuffled[i]]+"/fullimage")+"_back.png";
			...
			*/
			...
			//Cards in play
				...
				var div = ZA.createDiv(divCards,"roundResult","youWin");
				$(div).css({background:"url(_site/gameboard.png) -5px -1050px no-repeat"});
				var div = ZA.createDiv(divCards,"roundResult","youLose");
				$(div).css({background:"url(_site/gameboard.png) -125px -1050px no-repeat"});
				var div = ZA.createDiv(divCards,"roundResult","youDraw");
				$(div).css({background:"url(_site/gameboard.png) -260px -1050px no-repeat"});
				...
				$(div).html(
					...
					'<img src="_site/back.png" class="back" id="1" style="display:none;" />'+
				...
				$(div).html(
					'<div class="blank"><img src="_site/back.jpg" /></div>'+
					'<img src="_site/back.png" class="back" id="2" style="display:none;" />'+
			...
			//Cards Pool
				...
				$(div).css({
					width:"100%",
					height:"100%",
					background:"url(img/cards/thumb.jpg)",
					opacity:0.5
				});
					...
					if(ZA.getXML(tXML,"drawcards").length > 1)
					{
						...
						if(cards.length > 1)
						{
							...
							for(i=0; i<drawCards.length; i++)
							{
								...
								$(div).css({
									background:"url(img/cards/thumb0.jpg) no-repeat",
									top:0
								}).hide();
			...
			//Stat Selectors
				...
				for(var i=0; i<statscount; i++){
					...
					$(div).html(
						'<div class="statIndicator" style="right:-30px;"></div>'+
						'<div class="statSelectIndicator" style="right:-30px;"></div>'+
						'<div class="statSelectedIndicator" style="right:-30px;"></div>'
					);
					$(div).hover(function(){
				...
				for(var i=0; i<statscount; i++){
					...
					.addClass('noselect');
					$(div).html(
						'<div class="statIndicator" style="left:-30px;"></div>'+
						'<div class="statSelectIndicator" style="left:-30px;"></div>'+
						'<div class="statSelectedIndicator" style="left:-30px;"></div>'
					);
					$(div).hover(function(){
						if($(this).hasClass('noselect')){
							return false;
...
function WORK_MenuWindow()
{
	if(typeof WORK_MenuWindow._iInited=="undefined")
	{
		...
		WORK_MenuWindow.prototype.create=function(ID)
		{
			...
			$(div).css({
				background:"transparent url(_site/gameboard.png) -799px -780px no-repeat",
				width:109,
				marginTop:20,
				marginBottom:10
			});
...
WORK_Toptrumps.prototype.endRound=function(winner)
{
	...
	else
	{
		...
		if(winner=='1')
		{
			...
			var loser = '2';
			var result = "#youWin";
			var leftWinnerToWinnerStack = 0;
			var leftLoserToWinnerStack = -854;
			if(!ZA.sUsername){
				//freeplay
...
//			 \_____/_/    \_\_|  |_|______|  \____/   \/   |______|_|  \_\
			...
			function()
			{
				...
				if(ZT.p1score == 0){
					//user lost
					$("#gameMessage").html('<div style="width:225px;height:126px;background:url(_site/gameboard.png) -254px -875px no-repeat;position:relative;margin-left:auto;margin-right:auto;padding-top:45px;font-weight:bold;">'+score+'</div>');
				}
				else if(ZT.p2score == 0){
					//user won
					$("#gameMessage").html('<div style="width:225px;height:126px;background:url(_site/gameboard.png) -15px -874px no-repeat;position:relative;margin-left:auto;margin-right:auto;padding-top:45px;font-weight:bold;">'+score+'</div>');
