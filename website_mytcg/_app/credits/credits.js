function WORK_Credits()
{
	...
	{
		var container = ZA.createDiv(ZCR.divData);
		$(container).css({
			width:"100%",
			height:"100%"
		});
		...
		//current balance
		var balance = ZA.createDiv(divCredits);
		$(balance).css({
			width:300,
			height:86,
			top:40,
			...
			var title = ZA.createDiv(balance);
			$(title).css({
				top:-28,
				fontSize:16,
				fontWeight:"bold",
			...
			$(div).html(ZA.sUserCredits+' <span class="txtBlue">TCG</span>');
		...
		var divGateways = ZA.createDiv(container,"","paymentGateway");
		$(divGateways).css({
				...
				$(option).html(
					'<table>'+
						'<tr>'+
							...
							'</td>'+
							'<td class="credits"><label for="TCG5">350 <span class="txtBlue">TCG</span></label></td>'+
				...
				$(option).html(
					'<table>'+
						'<tr>'+
							...
							'</td>'+
							'<td class="credits"><label for="TCG10">700 <span class="txtBlue">TCG</span></label></td>'+
				...
				.html(
					'<table>'+
						'<tr>'+
							...
							'</td>'+
							'<td class="credits"><label for="TCG15">1050 <span class="txtBlue">TCG</span></label></td>'+
			...
			//payment methods
			var title = ZA.createDiv(divInner,"title");
			$(title).html('<span class="select">Select </span>Payment Method');
				...
				$(option).html(
					'<table>'+
						'<tr>'+
							...
							'<td><div style="position:relative;width:65px;height:16px;background:url(_site/all.png) -370px -205px no-repeat;"></div></td>'+
						'</tr>'+
					'</table>'+
				...
				$(option).html(
					'<table>'+
						'<tr>'+
							...
							'<td><div style="position:relative;width:65px;height:16px;background:url(_site/all.png) -300px -205px no-repeat;"></div></td>'+
						'</tr>'+
					'</table>'
				);
				...
				.html(
					'<table>'+
						'<tr>'+
							...
							'<td><div style="position:relative;width:112px;height:41px;background:url(_site/all.png) -300px -160px no-repeat;"></div></td>'+
						'</tr>'+
					'</table>'
				);
		...
		ZA.callAjax(ZCR.sURL+"?init=1&limit="+limit,function(xml)
		{
			...
			if(logsCount > 0)
			{
				...
				for(var i=0; i<logsCount; i++)
				{
					...
					var signcol = (parseInt(ZA.getXML(xml,"log_"+i+"/amount")) > 0) ? '#6c7927' : '#CC0000';
					tablebody+=
					'<tr class="'+rowclass+'">'+
				...
				transactions+=
				'<div style="position:relative;width:100%;color:#fff;background:#999;">'+
					...
					'</table>'+
				'</div>'+
				'<div style="position:relative;height:320px;width:626px;overflow:hidden;" id="logsHolder">'+
					'<table class="grid transactions" cellspacing="0" cellpadding="0">'+
...
'<div style="width:156px;height:15px;bottom:15px;left:15px;background:url(_site/all.png) -440px -180px no-repeat;"></div>'+
'</form>'
				);
				var icon = ZA.createDiv(form);
				$(icon).css({
					position:"relative",
					background:"url(_site/all.png) -270px -230px no-repeat",
					width:125,
					height:83,
					top:15,
					left:25
...
'</form>'
				...
				$(icon).css({
					...
					height:40,
					left:25,
					background:"url(_site/all.png) -450px -120px no-repeat"
				});
				//event handler
