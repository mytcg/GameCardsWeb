function WORK_Deck()
{
  ...
  this.deckHi = 16;
    ...
    WORK_Deck.prototype.buildDeckViewer=function(deckIndex, aXML)
    {
    	...
    	for(var i=0; i<deckCardLimit; i++)
        {
    		...
    		$(cardThumb).css({
    			...
    			backgroundImage:"url(_site/all_1.png)",
    			backgroundPosition:"-615px -119px",
    		});
    	...
    	$(userCardsHolder).html('<div style="width:100%;background:#D1D1D1;z-index:1000;top:-2px;padding-top:2px;" class="availableCardsH2"><h2>Available Cards: '+ZA.getXML(ZD.sXML,"decks/deck_"+deckIndex+"/category")+'</h2></div>');
    ...
    WORK_Deck.prototype.activateCardsScrollbar=function(){
		...
		$("#cardscontainer").css('width','570px');
		$("#availablecardholders").css('width','570px');
    };
    	...
    	$(".cardImage").draggable({
    		...
    		stop: function(event, ui){
    			...
    			if(ZD.remDropped)
    			{
					...
					ZA.callAjax("_app/deck/?remove=1&id="+usercard_id,function(reply){
	    				if(reply == '1')
	    				{
							//Adding removed card back to available cards
							var i = $("img[id='"+card_id+"']").attr('alt');
							var availHolder = $("#availablecardholder_"+i).find(".avail");
							if($(availHolder).size())
							{
		    					...
		    					else if(avail == 1){ $(availHolder).show(); }
		    					avail++;
    		...
    		stop: function(event, ui) {
    			...
    			if($(a).size())
    			{
    				if(ZD.addDropped)
    				{
    					...
    					ZA.callAjax("_app/deck/?add=1&cardid="+cardid+"&deckid="+ZD.iDeckID,function(usercardid){
    						if(usercardid != '0')
    						{
    ...
    WORK_Deck.prototype.activateAddzone=function(){
    	...
    	// Remove all previous droppable implementations
    	$("#deckcontainer").droppable("destroy");
    	
    	// Droppable area for available cards
    	// to add card to deck
		$("#deckcontainer").droppable(
		{
		    accept: ".thumb",
		    activeClass: "highlight",
		    hoverClass: "lightup",
		    drop: function(event, ui){
				...
				{
					if(ZD.deckCardCount(ui.draggable.attr('id')) > 0)
					{
						var icon = "-697px -63px";
						ZS.showWindow(icon,"Card already added to deck");
					}
					else
					{
						ZD.addDropped = true;
    ...
    WORK_Deck.prototype.buildListLarge=function(){
      ...
      $(topMenuMain).css({ top:0,left:0,width:720,height:23,"z-index":"9999",padding:5,});
      //Button: New
