...
if (isset($_GET['redeem'])){
	...
	if($success){
		switch ($redeemStatus['redeemType']){
			...
			break;
			case 2:
				$cards = openBooster("mytcg",$userID,$redeemStatus['target']);
...
if (intval($_GET["login"])==1){
    ...
    $ip = getip();
    $sUA=myqu("UPDATE mytcg_user SET last_useragent='".$sUA."',ip = '".$ip."', apps_id = 2, platform_id = 2 WHERE user_id='".$iValidUserID."'");
...
if (intval($_GET['init'])==1){
    ...
    echo $sTab.'<process val="'.$cps.'" />'.$sCRLF;
		...
		if(($aUser['freebie']=='0') || (is_null($aUser['freebie']))){
			...
			$starterPacks = myqu("SELECT P.product_id, P.description, CONCAT(IMG.description,'products/',P.image,'.png') AS 'image'
				FROM ".$pre."_product P
				JOIN ".$pre."_imageserver IMG ON P.full_imageserver_id = IMG.imageserver_id
				WHERE P.freebie = 1
  ...
  $sql = "SELECT category_id,description,parent_id,level "
    ."FROM ".$pre."_category "
    ."WHERE category_id > 17 AND category_id < 51 "
