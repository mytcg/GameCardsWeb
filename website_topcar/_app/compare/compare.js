function WORK_Compare()
{
	...
	WORK_Compare.prototype.buildScreen=function()
	{
		...
		$(leftCardHolder).css({
			width:250,
			height:350,
			background:"url(_site/back.png) no-repeat center center",
			top:55,
			right:35,
			"z-index":998
		}).hide();
		...
		$(rightCardHolder).css({
			width:250,
			height:350,
			background:"url(_site/back.png) no-repeat right",
			top:55,
			left:35,
			"z-index":998
		}).hide();
		...
		for(var i=0; i<count; i++){
			...
			$(thumb).html(
				'<img src="'+image+'_web.png" alt="'+description+'" />'+
	...
	{
		//display info
		$("#"+panel+"Back").attr('src',ZA.getXML(ZC.sXML,"cards/card_"+id+"/path")+'cards/'+ZA.getXML(ZC.sXML,"cards/card_"+id+"/image")+'_back.png');
		$("#"+panel+"Front").attr('src',ZA.getXML(ZC.sXML,"cards/card_"+id+"/path")+'cards/'+ZA.getXML(ZC.sXML,"cards/card_"+id+"/image")+'_front.png');
