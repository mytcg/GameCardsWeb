...
if(intval($_GET["cat"]) > 0){
  ...
  $query='SELECT DISTINCT C.category_id, CA.description  '
      ...
      .'WHERE UC.user_id = '.$userID.' AND UCS.description = "Album" AND CA.category_id > 17 AND CA.category_id < 51 '
...
if (intval($_GET['viewed']==1)){
  $query='UPDATE '.$pre.'_usercard SET is_new = 0 WHERE user_id = '.$userID;
  myqu($query);
}
...
$query='SELECT DISTINCT C.category_id, CA.description  '
  ...
  $query='SELECT C.card_id, C.image, C.description,I.description AS path, C.value, CA.category_id  '
        ...
        .'WHERE CA.category_id > 17 AND CA.category_id < 51 '
...
if(isset($_GET['auction']) && $_GET['auction']=='1')
{
	...
	if($market_id = myqu($sql))
	{
		...
		myqu("UPDATE mytcg_user SET credits = credits-".$auctionCost." WHERE user_id = ".$userID);
