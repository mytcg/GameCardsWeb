function WORK_Album(){
  ...
  this.pageCount = 0;
  this.perPage = 14;         //Hardcoded value
  ...
  this.aWindowAuction = null;
    ...
    WORK_Album.prototype.gotoPage=function(page){
      for (i=0;i<ZL.pageCount;i++){
        $(ZL.divScroll.childNodes[1].childNodes[i]).css({ backgroundPosition:"-130px -80px" }); 
      }
      $(ZL.divScroll.childNodes[1].childNodes[page]).css({ backgroundPosition:"-120px -80px" });
      if (ZA.aComponents[ZL.iComponentNo].iIsMaximized) {
        var newPos = page*-814;
      }else{
        var newPos = page*-646;
