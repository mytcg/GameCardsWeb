...
if (intval($_GET['init']==1))
{
	if(strlen($userID) > 0){
		$sql = "SELECT U.username AS 'owner', I.description AS 'imageserver', CA.description AS 'category', C.description, C.image, C.category_id, UC.usercard_id, UC.card_id, M.*,
				...
				WHERE M.markettype_id = 1 AND M.marketstatus_id = 1 AND CA.category_id > 17 AND CA.category_id < 51
...
if (isset($_GET['auction']))
{
	...
	$sql = "SELECT U.username AS 'owner', I.description AS 'imageserver', CA.description AS 'category', C.description, C.image, C.category_id, UC.card_id, M.* 
			...
			JOIN mytcg_category CA ON C.category_id = CA.category_id
			JOIN mytcg_user U ON M.user_id = U.user_id
			WHERE M.market_id = ".$market_id." AND CA.category_id > 17 AND CA.category_id < 51
