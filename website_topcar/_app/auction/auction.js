...
WORK_Auction.prototype.loadAuctions=function()
{
	...
	if(ZU.iCellCountAll > 0)
	{
		...
		while (iCount<(ZU.iCellCountAll))
		{
            ...
            $(textBox).css({ top:0,paddingTop:3,paddingRight:6,fontSize:9,lineHeight:"9px",width:138,height:30,color:"#FFF",right:5,textAlign:"right" });          
	...
	if(ZU.iCellCountAll > 0)
	{
		...
		while (iCount<ZU.iCellCountAll)
		{
			...
			$(divAuctionDetails).html(
				'<span style="color:#999;font-size:10px;">'+owner+'</span><br />'+
			...
			if(buyoutPrice != '0')
			{
				...
				$(div).css({
					...
					bottom:8,
					right:8,
					background:"url(_site/all_1.png) -275px -5px no-repeat"
				});
