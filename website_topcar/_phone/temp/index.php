...
if ($_GET['continuegame']) {
	...
	//set the user to waiting instead of pending
	myqu('UPDATE mytcg_gameplayer 
		SET gameplayerstatus_id = 2 
		WHERE user_id = '.$iUserID.' 
		AND game_id = '.$gameId);
	
	//then if both users are waiting, update the game phase
	$pendingUsers = myqu('SELECT * 
		FROM mytcg_gameplayer  
		WHERE gameplayerstatus_id = 1 
		AND game_id = '.$gameId);
			...
			if (sizeof($adminPlayerIdQuery) > 0) {
				...
				if ($aiIsActive == '1') {
					...
					$adminBestStatQuery = myqu('SELECT cs.cardstat_id, cs.categorystat_id, cs.description, cs.statvalue, ave.average, 
						...
						WHERE gpc.gameplayercard_id = 2254 
						ORDER BY picking_value DESC');
					
					//build xml with scores and explanation and send it back
					$sOP = selectStat($iUserId, $adminUserId, $gameId, $adminBestStatQuery[0]['categorystat_id']);
				}
			}
		}
	}
	
	//if the ai didnt make a move, load the game for the user
	if ($sOP != '') {
		$sOP = loadGame($gameId, $iUserID, $iHeight, $iWidth);
	}
...
function getUnplayedCards($iUserID, $gameId, $iHeight, $iWidth) {
	...
	while ($oneCard=$userCardDetails[$iCount]){
		...
		//gotta get the card stats
		$aStats=myqu('SELECT A.cardstat_id, A.description as des, B.description as val, statvalue, 
			A.left, top, width, height, frontorback 
			FROM mytcg_cardstat A, mytcg_categorystat B 
			WHERE A.categorystat_id = B.categorystat_id 
			AND A.card_id = '.$oneCard['card_id']);
		
		$iCountStat=0;
		$sOP.=$sTab.$sTab.'<stats>'.$sCRLF;
		While ($aOneStat=$aStats[$iCountStat]) {
			$sOP.=$sTab.$sTab.$sTab.'<stat desc="'.$aOneStat['val'].'" ival="'.$aOneStat['statvalue'].'" cardstat_id="'.$aOneStat['cardstat_id'].'"
				left="'.$aOneStat['left'].'" top="'.$aOneStat['top'].'" width="'.$aOneStat['width'].'" height="'.$aOneStat['height'].'" 
				frontorback="'.$aOneStat['frontorback'].'">'.$aOneStat['des'].'</stat>'.$sCRLF;
			$iCountStat++;
		}
		$sOP.=$sTab.$sTab.'</stats>'.$sCRLF;
		...
		//before setting the front and back urls, make sure the card is resized for the height
		$iHeight = resizeCard($iHeight, $iWidth, $oneCard['image']);
    ...
    $sOP.=$sTab.'<fronturl>'.$sFound.$iHeight.'/cards/'.$oneCard['image'].'_front.png</fronturl>'.$sCRLF;
    ...
    $sOP.=$sTab.'<backurl>'.$sFound.$iHeight.'/cards/'.$oneCard['image'].'_back.png</backurl>'.$sCRLF; 
		$iCount++;
	}
	$sOP.='</cards>'.$sCRLF;
	return $sOP;
...
function selectStat($userId, $oppUserId, $gameId, $statTypeId) {
	...
	$oppPlayerIdQuery = myqu('SELECT gameplayer_id 
		FROM mytcg_gameplayer 
		WHERE user_id = '.$oppUserId
		.' AND game_id = '.$gameId);
	...
	$cardStatDetails = myqu('SELECT MIN(gpc.pos) position, c.description card_name, cs.statvalue, cs.description statdescription, cats.description stattype
		FROM mytcg_gameplayercard gpc
		INNER JOIN mytcg_usercard uc
		ON uc.usercard_id = gpc.usercard_id
		INNER JOIN mytcg_card c
	...
	$oppCardDetails = myqu('SELECT MIN(gpc.pos) position, c.description card_name, cs.statvalue, cs.description statdescription, cats.description stattype
		FROM mytcg_gameplayercard gpc
		INNER JOIN mytcg_usercard uc
		ON uc.usercard_id = gpc.usercard_id
		INNER JOIN mytcg_card c
	...
	if ($oppStatValue > $userStatValue) {
		//the opponent won, set the score, last result and active player
		$exp = 'You won! Your '.$statType.' of '.$oppStatDescription.
			' beat your opponents '.$userCardName.' with a '.$statType.
			' of '.$userStatDescription.'.';
		myqu('UPDATE mytcg_gameplayer SET score = (score + 1), last_result = '.$exp.', is_active = 1, gameplayerstatus_id = '.$oppStatusId.' WHERE gameplayer_id = '.$oppPlayerId);
		
		$exp = 'You lost! Your opponent`s '.$oppCardName.' had a '.$statType.
			' of '.$oppStatDescription.'.';
		myqu('UPDATE mytcg_gameplayer SET last_result = '.$exp.', is_active = 0, gameplayerstatus_id = 1 WHERE gameplayer_id = '.$userPlayerId);
		...
		//the user won, set the score, last result and active player
		$exp = 'You lost! Your opponent`s '.$userCardName.' had a '.$statType.
			' of '.$userStatDescription.'.';
		myqu('UPDATE mytcg_gameplayer SET last_result = '.$exp.', is_active = 0, gameplayerstatus_id = '.$oppStatusId.' WHERE gameplayer_id = '.$oppPlayerId);
		
		$exp = 'You won! Your '.$statType.' of '.$userStatDescription.
			' beat your opponents '.$oppCardName.' with a '.$statType.
			' of '.$oppStatDescription.'.';
		myqu('UPDATE mytcg_gameplayer SET score = (score + 1), last_result = '.$exp.', is_active = 1, gameplayerstatus_id = 1 WHERE gameplayer_id = '.$userPlayerId);
		...
		//it was a draw
		myqu('UPDATE mytcg_gameplayer SET score = (score + 1) WHERE gameplayer_id = '.$oppPlayerId);
		myqu('UPDATE mytcg_gameplayer SET score = (score + 1) WHERE gameplayer_id = '.$userPlayerId);
		
		$exp = 'Draw! Your opponent`s '.$statType.' of '.$oppStatDescription.
			' equals your`s!';
		myqu('UPDATE mytcg_gameplayer SET score = (score + 1), last_result = '.$exp.', gameplayerstatus_id = 1 WHERE gameplayer_id = '.$userPlayerId);
		myqu('UPDATE mytcg_gameplayer SET last_result = '.$exp.', gameplayerstatus_id = '.$oppStatusId.' WHERE gameplayer_id = '.$oppPlayerId);
...
if ($_GET['newgame']) {
	...
	myqu('INSERT INTO mytcg_gameplayer (game_id, user_id, is_active, score, gameplayerstatus_id)
		VALUES ('.$gameId.', '.$iUserID.', 1, 0, 1)'); //gameplayerstatus_id 1 is pending, so they need to make the move
	myqu('INSERT INTO mytcg_gameplayer (game_id, user_id, is_active, score, gameplayerstatus_id)
		VALUES ('.$gameId.', '.$adminUserId.', 0, 0, 2)');//gameplayerstatus_id 2 is waiting, so waiting for the other player to move
