...
function openStarter($userID,$packID){
  ...
  for ($i = 0; $i < $iNumCards; $i++){
    ...
    $aCards=myqui("INSERT INTO mytcg_usercard (user_id, card_id, is_on_auction) VALUES ({$userID},{$iCardID},0)");
		
		if ($aGetCards[$i]['quality_id'] > $quality) {
			$quality = $aGetCards[$i]['quality_id'];
...
if ($iUserCardID = $_GET['createauction']){
  ...
  $aUpdate=myqui('UPDATE mytcg_usercard SET status="1" '
...
if ($_GET['buyproduct']){
  ...
  //GET PRODUCT DETAILS
  $aDetails=myqu('SELECT A.product_id, A.product_name, A.product_thumb_image, A.product_s_desc, '
    .'B.product_currency, B.product_price,A.product_sku, P.pack_type '
    .'FROM mytcg_product A '
    .'INNER JOIN mytcg_product_price B '
    .'ON A.product_id=B.product_id '
    .'INNER JOIN mytcg_pack P '
    .'ON A.product_id=P.vm_product_id '
  ...
  //Total order cost
  $itemCost = $aDetails[0]['product_price'];
  ...
  if ($bValid)
  {
    ...
    elseif($aDetails[0]['pack_type'] == "booster"){
      $iCardID = openBooster($iUserID,$iProductID);
		...
		$aServers=myqu('SELECT parameter, value '
			.'FROM mytcg_system '
			.'WHERE category="image_server" '
			.'AND height<="'.$iHeight.'" '
			.'AND width<="'.$iWidth.'" '
    ...
    $aCardDetails=myqu("SELECT c.card_id,c.description,c.server_image_front,c.server_image_back,cq.quality_name,c.image_id
												FROM mytcg_card c
												INNER JOIN mytcg_card_quality AS cq
												ON (c.quality_id = cq.quality_id)
												WHERE c.card_id=".$iCardID."
												ORDER BY c.quality_id DESC");
		
		$cardCount = sizeof($aCardDetails);
		echo $sTab.'<card>'.$sCRLF;
		echo $sTab.$sTab.'<id>'.$aCardDetails[0]['card_id'].'</id>'.$sCRLF;
		echo $sTab.$sTab.'<image_id>'.$aCardDetails[0]['image_id'].'</image_id>'.$sCRLF;
		...
		echo $sTab.$sTab.'<count>'.$cardCount.'</count>'.$sCRLF;
		
		$sFound='';
		$iCountServer=0;
		while ((!$sFound)&&($aOneServer=$aServers[$iCountServer])){
			if ($aOneServer['parameter']==$aCardDetails[0]['server_image_front']){
				$sFound=$aOneServer['value'];
			} else {
				$iCountServer++;
			}
		}
		echo $sTab.$sTab.'<urlfront>'.$sFound.$aCardDetails[0]['image_id'].'_front.jpg</urlfront>'.$sCRLF;
		
		$sFound='';
		$iCountServer=0;
		while ((!$sFound)&&($aOneServer=$aServers[$iCountServer])){
			if ($aOneServer['parameter']==$aCardDetails[0]['server_image_back']){
				$sFound=$aOneServer['value'];
			} else {
				$iCountServer++;
			}
		}
		echo $sTab.$sTab.'<urlback>'.$sFound.$aCardDetails[0]['image_id'].'_back.jpg</urlback>'.$sCRLF;
		echo $sTab.'</card>'.$sCRLF;
  } 
  exit;
}
...
if ($_GET['auctionbid']){
  ...
  $credits = $result[0]['credits'];
	...
	if ($credits >= $bid) {
		...
		$query = "INSERT INTO mytcg_auctionbid (auctioncard_id, user_id, price, timestamp) VALUES (".$auctionCardId
			.", ".$iUserID.", ".$bid.", now())";
		myqu($query);
...
if ($_GET['buyauctionnow']){
  ...
  $credits = $result[0]['credits'];
	...
	if ($credits >= $buyNowPrice) {
		...
		$query = "update mytcg_usercard set status = '0', user_id = ".$iUserID." where usercard_id = ".$userCardId;
		myqu($query);
...
if ($iCategory=$_GET['cardsincategory']){
	...
	$aCards=myqu('SELECT A.card_id, A.status, count(*) quantity, B.image_id, A.usercard_id, '
		.'B.description, B.server_image_thumb, B.server_image_front, B.server_image_back '
		.'FROM mytcg_card B '
		.'INNER JOIN mytcg_usercard A '
		.'ON A.card_id=B.card_id '
		.'WHERE A.user_id="'.$iUserID.'" '
		.'AND B.category_id="'.$iCategory.'" '
		.'AND A.status=0 '
		.'GROUP BY B.card_id '
		.'UNION '
		.'SELECT B.card_id, "0" as status, 0 as quantity, B.image_id, "0" as usercard_id, '
		.'B.description, B.server_image_thumb, B.server_image_front, B.server_image_back '
		.'FROM mytcg_card B '
		.'WHERE B.category_id="'.$iCategory.'" '
		.'AND B.card_id NOT IN (SELECT card_id from mytcg_usercard where user_id = "'.$iUserID.'"  and status = 0) '
		.'GROUP BY B.card_id '
	...
	while ($aOneCard=$aCards[$iCount]){
		...
		$iCountServer=0;
		while ((!$sFound)&&($aOneServer=$aServers[$iCountServer])){
			if ($aOneServer['parameter']==$aOneCard['server_image_thumb']){
				$sFound=$aOneServer['value'];
			} else {
				$iCountServer++;
			}
		}
		$sOP.=$sTab.'<thumburl>'.$sFound.$aOneCard['image_id'].'_thumb.jpg</thumburl>'.$sCRLF;
		$sFound='';
		$iCountServer=0;
		while ((!$sFound)&&($aOneServer=$aServers[$iCountServer])){
			if ($aOneServer['parameter']==$aOneCard['server_image_front']){
				$sFound=$aOneServer['value'];
			} else {
				$iCountServer++;
			}
		}
		$sOP.=$sTab.'<fronturl>'.$sFound.$aOneCard['image_id'].'_front.jpg</fronturl>'.$sCRLF;
		$sFound='';
		$iCountServer=0;
		while ((!$sFound)&&($aOneServer=$aServers[$iCountServer])){
			if ($aOneServer['parameter']==$aOneCard['server_image_back']){
				$sFound=$aOneServer['value'];
			} else {
				$iCountServer++;
			}
		}
		$sOP.=$sTab.'<backurl>'.$sFound.$aOneCard['image_id'].'_back.jpg</backurl>'.$sCRLF;
		$iCount++;
	}
	$sOP.='</cardsincategory>'.$sCRLF;
	header('xml_length: '.strlen($sOP));
...
if ($_GET['categoryauction']){
	...
	$aAuctionCards=myqu('SELECT AC.auctioncard_id, UC.usercard_id, UC.card_id, C.image_id, C.server_image_thumb, C.description, '
		.'AC.opening_bid, AC.buy_now_price, MAX(AB.price) price, U.username, UB.username as last_bid_username, date_format(AC.datetime_end, "%Y-%m-%d") as end_date '
		...
		.'GROUP BY AC.auctioncard_id, UC.card_id, C.image_id, C.server_image_thumb, C.description, '
		.'AC.opening_bid, AC.buy_now_price, UC.usercard_id, U.username, UB.username, AC.datetime_end '
...
if ($_GET['usercategories']){
	$aCategories=myqu('SELECT DISTINCT A.category_id, B.description 
		FROM mytcg_card A 
		INNER JOIN mytcg_category B 
		ON A.category_id=B.category_id 
		INNER JOIN mytcg_usercard C 
		ON A.card_id=C.card_id 
		INNER JOIN mytcg_usercardstatus S 
		ON S.usercardstatus_id=C.usercardstatus_id 
		WHERE C.user_id='.$iUserID.' 
		AND S.description = "default" ');
...
if ($_GET['categoryproducts']){
	...
	$aProducts=myqu('SELECT DISTINCT P.PRODUCT_ID, P.DESCRIPTION, T.DESCRIPTION PACK_TYPE, '
		.'P.PRICE, CONCAT(I.DESCRIPTION , "products/" , P.IMAGE , "_thumb.png") IMAGEURL, '
		.'M.NO_OF_CARDS, (CASE WHEN SUM(P.IN_STOCK) IS NULL THEN 0 ELSE SUM(P.IN_STOCK) END) AS releasedleft '
		.'FROM mytcg_category C, mytcg_imageserver I, '
		.'mytcg_productcategory_x PC, mytcg_productspacktype T, '
		.'mytcg_productspack M, mytcg_product P '
