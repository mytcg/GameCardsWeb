...
function objPlayerBar(){
  ...
  objPlayerBar.prototype.create=function(credits,xp){
	...
	if (ZA.sUsername) {
    	...
    	$(objBar.divCredits).css({zIndex:2,left:15,width:120,height:13,bottom:"25%",textShadow:"0 1px 1px #d1d1d1"});
    	$(objBar.divCredits).html("<span>YOU HAVE: </span><span id='spanCreditsID' class='txtRed'>"+objBar.iCredits+"</span><span class='txtRed'> TCG</span></div");
	}
  ...
  objPlayerBar.prototype.update=function(objProps){
    if (objProps.credits){
      ...
      $(ZA.oPlayerBar.spanCredits).delay(600).animate({color:"#CC0000"},1500);
    }
    if (objProps.xp){
...
function WORK_App(){
   ...
   this.imgAll="_site/all_skin.png";
	...
	this.iWidthContainer=790;
	this.iHeightContainer=845;
...
WORK_App.prototype.createPage=function(){
	...
	if (!ZA.sUsername) {
		...
		var divLeftSms=ZA.createDiv(divLeft,"left_banner_sms_app","");
	} else {
		//logged in
...
WORK_App.prototype.init=function(sXMLInit){
  ...
  /////////////////////////
	...
	ZA.iWidthContainer=parseInt("790px");
	ZA.iHeightContainer=parseInt("845px");
	...
	$(divHeader).css({height:67,opacity:1});
	...
	var divFooterBlock=ZA.createDiv(divFooterCon,"bodyfooter_block");
	$(divFooterBlock).html("TopCar Website Version 1.0");
	//ZM.createBottom();
	if(ZA.sUsername){
...
WORK_Menu.prototype.createTop=function(){
	...
	// logged out
	if (!ZA.sUsername) {
		ZA.sMenuItemsTop = "Login|Auction|Buy Cards|How it works";
	}
	// logged in
	else{
