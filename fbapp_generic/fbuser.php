<?php
	...
	if(($_SESSION['request_ids']!="")&&($fbuser != "")){
		...
		if($aRequest[0]['request_status'] == 1){
			...
			$res = myqu($sql);
			if(sizeof($res) > 0){
				myqu("UPDATE mytcg_user SET credits = credits + 10 WHERE user_id=".$res[0]['user_id']);	
				...
				VALUES ('.$res[0]['user_id'].', "Received 10 credits for referring '.$iGotIT[0]['username'].'", now(), 10)');
			}
