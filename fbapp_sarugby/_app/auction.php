...
if (isset($_GET['bid']))
{
	...
	else
	{
	  ...
	  if ($selectId == $userID) {
		...
		if(sizeof($lastBidderQuery) > 0){
			foreach($lastBidderQuery as $lastBidder){
				if ($bid_amount <= $lastBidder['price']) {
					//user has insuffient credits to place the bid
					echo '<result>'.$sCRLF;
					echo $sTab.'<value val="0" />'.$sCRLF;
		...
		myqu("INSERT INTO tcg_transaction_log (fk_user, fk_boosterpack, fk_usercard, fk_card, transaction_date, description, tcg_credits, fk_payment_channel, application_channel, mytcg_reference_id, fk_transaction_type)
			VALUES(".$userID.", NULL, (SELECT usercard_id FROM mytcg_market WHERE market_id = ".$market_id."), (SELECT card_id FROM mytcg_usercard a, mytcg_market b WHERE a.usercard_id = b.usercard_id AND market_id = ".$market_id."), 
					now(), 'Placed bid of ".$bid_amount." credits on ".$carName."', -".$bid_amount.", NULL, 'facebook',  (SELECT max(transaction_id) FROM mytcg_transactionlog WHERE user_id = ".$userID."), 7)");
		
		//place the bid
		$sql = "INSERT INTO mytcg_marketcard (market_id, user_id, price, date_of_transaction)
				VALUES (".$market_id.",$userID,$bid_amount,now());";
		myqu($sql);
...
if(isset($_GET['create']))
{
	...
	//if ($userCredits > $auctionCost) {
		if($market_id = myqu($sql))
		{
			...
			myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
					VALUES(".$userID.", 'Spent ".$auctionCost." credits on creating auction for ".$uc[0]['description']."', NOW(), -".$auctionCost.")");
			...
			$sql = "INSERT INTO tcg_transaction_log (fk_user, fk_boosterpack, fk_usercard, fk_card, transaction_date, description, tcg_credits, fk_payment_channel, application_channel, mytcg_reference_id, fk_transaction_type)
					VALUES(".$userID.", NULL, ".$usercard_id.", (SELECT card_id FROM mytcg_usercard WHERE usercard_id = ".$usercard_id."), 
							now(), 'Spent ".$auctionCost." credits on creating auction for ".$uc[0]['description']."', -".$auctionCost.", NULL, 'facebook',  (SELECT max(transaction_id) FROM mytcg_transactionlog WHERE user_id = ".$userID."), 6)";
			myqu($sql);
			*/
			//Get user credits
			$userQuery = myqu("SELECT premium FROM mytcg_user WHERE user_id=".$userID);
...
if (isset($_GET['buy']))
{
	...
	if(sizeof($lastBidderQuery) > 0){
		foreach($lastBidderQuery as $lastBidder){
			...
			//add transaction log
			myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
					VALUES(".$lastBidder['user_id'].", 'Refunded with ".$lastBidder['price']." credits for bid on bought out ".$carName."', NOW(), ".$lastBidder['price'].")");
			...
			myqu("INSERT INTO tcg_transaction_log (fk_user, fk_boosterpack, fk_usercard, fk_card, transaction_date, description, tcg_credits, fk_payment_channel, application_channel, mytcg_reference_id, fk_transaction_type)
					VALUES(".$lastBidder['user_id'].", NULL, (SELECT usercard_id FROM mytcg_market WHERE market_id = ".$marketID."), (SELECT card_id FROM mytcg_usercard a, mytcg_market b WHERE a.usercard_id = b.usercard_id AND market_id = ".$marketID."), 
							now(), 'Refunded with ".$lastBidder['price']." credits for bid on bought out ".$carName."', ".$lastBidder['price'].", NULL, 'facebook',  (SELECT max(transaction_id) FROM mytcg_transactionlog WHERE user_id = ".$lastBidder['user_id']."), 8)");
	...
	//Record transaction
	$sql = "INSERT INTO mytcg_marketcard (market_id, usercard_id, price, date_of_transaction, user_id) ".
			"VALUES(".$marketID.",".$usercardID.",".$price.",'".$now."',".$userID.");";
	$temp[]=$sql;
	myqu($sql);
	
	//Subtract credits from buyer
	$sql = "UPDATE mytcg_user SET premium=(premium-".$price.") WHERE user_id=".$userID.";";
	$temp[]=$sql;
	myqu($sql);
	//Add transaction log
	myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
	...
	myqu("INSERT INTO tcg_transaction_log (fk_user, fk_boosterpack, fk_usercard, fk_card, transaction_date, description, tcg_credits, fk_payment_channel, application_channel, mytcg_reference_id, fk_transaction_type)
			VALUES(".$userID.", NULL, ".$usercardID.", (SELECT card_id FROM mytcg_usercard WHERE usercard_id = ".$usercardID."), 
					now(), 'Spent ".$price." credits on buyout of ".$carName."', -".$price.", NULL, 'facebook',  (SELECT max(transaction_id) FROM mytcg_transactionlog WHERE user_id = ".$userID."), 13)");

	
	//Add credits to seller
	$sql = "UPDATE mytcg_user SET premium=(premium+".$price.") WHERE user_id=".$oldowner.";";
	$temp[]=$sql;
	myqu($sql);
	//Add transaction log
	myqu("INSERT INTO mytcg_transactionlog (user_id, description, date, val)
	...
	myqu("INSERT INTO tcg_transaction_log (fk_user, fk_boosterpack, fk_usercard, fk_card, transaction_date, description, tcg_credits, fk_payment_channel, application_channel, mytcg_reference_id, fk_transaction_type)
			VALUES(".$oldowner.", NULL, ".$usercardID.", (SELECT card_id FROM mytcg_usercard WHERE usercard_id = ".$usercardID."), 
				now(), 'Received ".$price." credits for buyout on ".$carName."', ".$price.", NULL, 'facebook',  (SELECT max(transaction_id) FROM mytcg_transactionlog WHERE user_id = ".$oldowner."), 14)");
...
if(isset($_GET['filter'])){
	...
	$query= "SELECT * FROM
		(SELECT U.username AS 'owner', I.description AS 'imageserver', CA.description AS 'category', C.description AS cardname, C.image, C.category_id, UC.user_id AS uid, UC.usercard_id AS ucid, UC.card_id AS uccid, M.*,
